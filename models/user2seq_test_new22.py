...
class user2seq_test_new22(nn.Module):

    def __init__(self, config, vocab, use_cuda, use_content=False, pretrain=None):
        super(user2seq_test_new22, self).__init__()
        self.vocab = vocab
        ...
        self.z_to_hidden = nn.Linear(config.n_z, config.decoder_hidden_size)
        self.gama_kld_cvae = config.gama_kld_cvae
        # prior
        self.prior_fc1_layer = nn.Linear(config.n_z + config.decoder_hidden_size, config.n_z * 2)
    ...
    def compute_loss(self, out_dict, targets):
        ...
        # kld cvae
        kld_cvae = out_dict['kld_cvae']
        loss += self.gama_kld_cvae * kld_cvae
    ...
    def encode(self, batch, is_test=False):
        ...
        return contexts, state, z, kld, mu, logvar

    def forward(self, batch, use_cuda):
        if use_cuda:
            batch = move_to_cuda(batch)
        bow_mask = (batch.tgt_bow > 0).float()
        contexts, state, z, kld, mu, logvar = self.encode(batch)
        ...
        # prior
        news_rep = state[0][-1]
        prior_mulogvar = self.prior_mulogvar_layer(torch.tanh(self.prior_fc1_layer(torch.cat([user, news_rep], dim=-1))))
        prior_mu, prior_logvar = torch.chunk(prior_mulogvar, 2, dim=1)

        # combine user and state
        combine_state = self.z_to_hidden(z)
        zz = combine_state.unsqueeze(0).repeat(self.config.num_layers, 1, 1)
        state = (zz, zz)

        # kld between post z and prior z
        # for loss
        ...
        kld_cvae = gaussian_kld(mu, logvar, prior_mu, prior_logvar)
