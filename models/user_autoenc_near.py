...
class GetUser(nn.Module):
    ...
    def forward(self, latent_context, is_test=False):
        if not is_test:
            ...
            l2_inputs = torch.sum(latent_context ** 2, -1, keepdim=True)
            l2_embeddings = torch.sum(self.use_emb.weight ** 2, -1)
            l2_embeddings = torch.unsqueeze(l2_embeddings, 0)
            embeddings = torch.transpose(self.use_emb.weight, 0, 1)
            dot = torch.matmul(latent_context, embeddings)
            distance = l2_inputs + l2_embeddings - 2 * dot

            selected_user = torch.argmin(distance, -1)
...
class user_autoenc_near(nn.Module):
    ...
    def forward(self, batch, use_cuda):
        ...
        # decoder
        h_user = comment_rep + (h_user - comment_rep).detach()
