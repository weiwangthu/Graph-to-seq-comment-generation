...
def criterion(tgt_vocab_size, use_cuda):
    ...
    weight[vocab.PAD] = 0
    crit = nn.CrossEntropyLoss(weight, ignore_index=vocab.UNK, reduction='sum')
    if use_cuda:
        crit.cuda()
    return crit
...
def memory_efficiency_cross_entropy_loss(hidden_outputs, targets, criterion, config):
    ...
    for i, (out_t, targ_t) in enumerate(zip(outputs_split, targets_split)):
        ...
        num_correct_t = pred_t.data.eq(targ_t.data).masked_select(targ_t.ne(vocab.PAD).data).sum()
        num_total_t = targ_t.ne(vocab.PAD).data.sum()
...
def cross_entropy_loss(hidden_outputs, targets, criterion):
    ...
    num_correct = pred.data.eq(targets.data).masked_select(targets.ne(vocab.PAD).data).sum()
    num_total = targets.ne(vocab.PAD).data.sum()
