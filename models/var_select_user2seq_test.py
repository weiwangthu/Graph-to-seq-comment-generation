...
class var_select_user2seq_test(nn.Module):

    def __init__(self, config, vocab, use_cuda, use_content=False, pretrain=None):
        super(var_select_user2seq_test, self).__init__()
        self.vocab = vocab
    ...
    def forward(self, batch, use_cuda):
        ...
        # get user
        h_user, selected_user = self.get_user(comment_rep)
        # user loss
