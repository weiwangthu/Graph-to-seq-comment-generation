...
class var_select_user2seq_new22(nn.Module):

    def __init__(self, config, vocab, use_cuda, use_content=False, pretrain=None):
        super(var_select_user2seq_new22, self).__init__()
        self.vocab = vocab
    ...
    def compute_loss(self, out_dict, targets):
        ...
        loss += self.config.gama1 * gate_loss
    ...
    def encode(self, batch, is_test=False):
        ...
        return contexts, state, z, kld, comment_rep, context_gates

    def forward(self, batch, use_cuda):
        if use_cuda:
            batch = move_to_cuda(batch)
        bow_mask = (batch.tgt_bow > 0).float()
        contexts, state, z, kld, comment_rep, context_gates = self.encode(batch)
        ...
        init_state = (contexts * content_mask.float().unsqueeze(dim=2)).sum(dim=1) / content_len.float().unsqueeze(dim=1)
        ...
        pri_gates = (context_gates.squeeze(dim=-1) * content_mask.float()).sum(dim=-1) / content_len.float()
        return {
            'outputs': outputs,
