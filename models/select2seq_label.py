...
class select2seq_label(nn.Module):
    ...
    def compute_loss(self, out_dict, targets):
        ...
        label_loss = out_dict['label_loss']

        loss = word_loss[0] + self.config.gama1 * gate_loss + self.config.gama_label * label_loss
        return {
            'loss': loss,
    ...
    def encode(self, batch, is_test=False):
        ...
        # if not is_test:
        context_gates = gumbel_softmax(torch.log(org_context_gates), self.config.tau)
        context_gates = context_gates[:, :, 0]  # bsz * n_context
        org_context_gates = org_context_gates[:, :, 0]  # bsz * n_context
        # contexts = contexts * gates
        return contexts, state, context_gates, org_context_gates

    def forward(self, batch, use_cuda):
        if use_cuda:
            batch = move_to_cuda(batch)
        contexts, state, context_gates, org_context_gates = self.encode(batch)
        ...
        label_count = 0
        pos_log_gate = -torch.log(org_context_gates + 1e-10)
        neg_log_gate = -torch.log(1 - org_context_gates + 1e-10)

        for ii, item in enumerate(content_label):
            if item[0] != -1:
                pos_word = torch.zeros_like(content_mask[0])
                pos_word[item] = 1
                item_gate = pos_log_gate[ii][pos_word]
                label_loss += item_gate.sum()

                neg_word_temp = ~pos_word
                neg_word = content_mask[ii] & neg_word_temp
                item_neg_gate = neg_log_gate[ii][neg_word]
                label_loss += item_neg_gate.sum()

                label_count += content_len[ii]
        if label_count > 0:
            label_loss = label_loss / label_count
    ...
    def beam_sample(self, batch, use_cuda, beam_size=1, n_best=1):
        ...
        if use_cuda:
            batch = move_to_cuda(batch)
        contexts, enc_state, org_context_gates, _ = self.encode(batch, True)
