...
class var_select2seq_test_span3(nn.Module):

    def __init__(self, config, vocab, use_cuda, use_content=False, pretrain=None):
        super(var_select2seq_test_span3, self).__init__()
        self.vocab = vocab
        ...
        self.select_word_gate = SelectGate(config)
    ...
    def compute_loss(self, out_dict, targets):
        ...
        return {
            ...
            'w_gates': out_dict['w_gates'],
        }

    def merge_local_context(self, group_vectors, group_masks, w_masks):
        g_vs = []
        g_mask = []
        for i in range(len(group_vectors)):
            # mask empty span
            no_pad_len = group_masks[i].float().sum(dim=-1)
            g_mask.append(no_pad_len > 0)

            # mask average to get group vector
            co_mask = w_masks[i] * group_masks[i].float()
            g_v = (group_vectors[i] * co_mask.unsqueeze(dim=2)).sum(dim=1)
            g_len = co_mask.sum(dim=-1) + 0.00001
    ...
    def encode(self, batch, is_test=False):
        ...
        org_word_context_gates = self.select_word_gate(contexts, title_rep)  # output: bsz * n_context * 2
        word_context_gates = gumbel_softmax(torch.log(org_word_context_gates + 1e-10), self.config.tau)
        w_mask = word_context_gates[:, :, 0]  # bsz * n_context
        org_word_context_gates = org_word_context_gates[:, :, 0]  # bsz * n_context
        temp_mask = w_mask * content_mask.float()
        w_gates = (temp_mask.sum(dim=-1) / content_len.float()).mean()
        ...
        local_w_mask = torch.split(w_mask, self.config.content_span, dim=1)
        local_vectors, local_vector_mask = self.merge_local_context(local_contexts, local_mask, local_w_mask)
        ...
        # merge word mask and span mask
        context_gates = context_gates * w_mask
        post_context_gates = post_context_gates * w_mask

        return contexts, state, post_context_gates, comment_rep, kld_select, context_gates, l1_gates, pri_gates, w_gates

    def forward(self, batch, use_cuda):
        if use_cuda:
            batch = move_to_cuda(batch)
        contexts, state, post_context_gates, comment_rep, kld_select, context_gates, l1_gates, pri_gates, word_context_gates = self.encode(batch)
