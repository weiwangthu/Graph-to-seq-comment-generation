...
class autoenc_vae_cat(nn.Module):
    ...
    def __init__(self, config, vocab, use_cuda, use_content=False, pretrain=None):
        ...
        self.decoder = models.rnn_cat_decoder(config, self.vocab_size, embedding=self.embedding)
        self.config = config
    ...
    def forward(self, batch, use_cuda):
        ...
        # decoder
        outputs, final_state, attns = self.decoder(tgt[:, :-1], init_state, z, None, None)
