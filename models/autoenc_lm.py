...
class autoenc_lm(nn.Module):
    ...
    def sample(self, batch, use_cuda):
        ...
        bos = torch.ones(init_state[0].size(1)).long().fill_(self.vocab.word2id('[START]'))
        bos = bos.to(init_state[0].device)
        sample_ids, final_outputs = self.decoder.sample([bos], init_state, None, None)
