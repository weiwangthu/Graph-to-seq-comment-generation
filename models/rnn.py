...
class rnn_gate_decoder(nn.Module):

    def __init__(self, config, vocab_size, embedding=None):
        super(rnn_gate_decoder, self).__init__()
        if embedding is not None:
            self.embedding = embedding
        else:
            self.embedding = nn.Embedding(vocab_size, config.emb_size)
        ...
        self.attention = models.global_gate_attention(config.decoder_hidden_size, activation)
        # self.attention = models.global_attention(config.decoder_hidden_size, activation)
        self.hidden_size = config.decoder_hidden_size
        self.dropout = nn.Dropout(config.dropout)
        self.config = config
...
class rnn_topic_decoder(nn.Module):
    ...
    def forward(self, inputs, init_state, contexts=None, context_gates=None, topic=None):
        embs = self.embedding(inputs)
        outputs, state, attns = [], init_state, []
        for emb in embs.split(1, dim=1):
            output, state = self.rnn(emb.squeeze(1), state)
            attn_weights = None
            if contexts is not None:
                output, attn_weights = self.attention(output, contexts, context_gates=context_gates, topic=topic)
            output = self.dropout(output)
            outputs += [self.linear(output)]
            attns += [attn_weights]
        outputs = torch.stack(outputs, 1)
...
class rnn_topic2_decoder(nn.Module):

    def __init__(self, config, vocab_size, embedding=None):
        super(rnn_topic2_decoder, self).__init__()
        if embedding is not None:
            self.embedding = embedding
        else:
            self.embedding = nn.Embedding(vocab_size, config.emb_size)
        ...
        self.attention = models.global_attention_with_topic(config.decoder_hidden_size, config.decoder_hidden_size + config.n_z, activation)
        # self.attention = models.global_attention(config.decoder_hidden_size, activation)
        self.hidden_size = config.decoder_hidden_size
        self.dropout = nn.Dropout(config.dropout)
        self.config = config
