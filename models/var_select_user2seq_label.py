...
class var_select_user2seq_label(nn.Module):

    def __init__(self, config, vocab, use_cuda, use_content=False, pretrain=None):
        super(var_select_user2seq_label, self).__init__()
        self.vocab = vocab
    ...
    def compute_loss(self, out_dict, targets):
        ...
        loss += self.config.gama_label * label_loss
    ...
    def encode(self, batch, is_test=False):
        ...
        return contexts, state, z, kld, post_context_gates, comment_rep, kld_select, context_gates, org_context_gates

    def forward(self, batch, use_cuda):
        if use_cuda:
            batch = move_to_cuda(batch)
        bow_mask = (batch.tgt_bow > 0).float()
        contexts, state, z, kld, post_context_gates, comment_rep, kld_select, context_gates, org_context_gates = self.encode(batch)
